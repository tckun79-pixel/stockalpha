...
with st.sidebar:
    ...
    mmap = {"Auto-detect": "Auto", "US (NYSE/NASDAQ)": "US", "SG (SGX)": "SG", "HK (HKEX)": "HK"}
...
if not run_btn or not ticker_input:
    ...
    <div style="text-align:center;padding:60px 20px 40px">
      <div style="font-size:2.2rem;font-weight:700;color:#e8eaf0;letter-spacing:-.03em;margin-bottom:12px">
        Deep Stock Analysis</div>
      <div style="font-size:1rem;color:#8b8fa8;max-width:480px;margin:0 auto 32px">
        5-quarter fundamental · 5-year technical · 6–12 month forecast
      </div>
    ...
    for col, icon, title, desc in [
        (c1, "📊", "Fundamental", "Revenue, EPS, margins, FCF, valuation ratios and red flag detection over 5 quarters."),
        (c2, "📈", "Technical",   "MACD, RSI, Bollinger Bands, SMA/EMA, ATR, OBV, support & resistance levels."),
        (c3, "🔮", "Forecast",    "Ensemble model (Linear Regression + Bollinger + Forward P/E) for 6M & 12M targets."),
    ]:
        with col:
            st.markdown(f"""
...
with st.spinner("⚙️ Running fundamental analysis..."):
    fund = compute_quarterly_metrics(data["income_stmt"], data["balance_sheet"], data["cashflow"], info)
...
# ── TABS ───────────────────────────────────────────────────────────────────────
tab1, tab2, tab3 = st.tabs(["📊 Fundamental", "📈 Technical", "🔮 Forecast (6–12M)"])
...
with tab1:
    ...
    if quarters:
        ...
        with c1:
            ...
            render_metric_card("Latest Revenue", rv, f"{latest.get('Rev YoY%','—')}% YoY",
                               (latest.get("Rev YoY%") or 0) > 0)
        with c2:
            eps = latest.get("EPS Diluted")
            render_metric_card("EPS (Diluted)", f"{eps:.2f}" if eps else "—",
                               f"Basic: {latest.get('EPS Basic','—')}")
        with c3:
            nm = latest.get("Net Margin%")
            render_metric_card("Net Margin", f"{nm:.1f}%" if nm else "—",
                               f"Op: {latest.get('Op Margin%','—')}%", (nm or 0) > 0)
        with c4:
            pe = valuation.get("P/E (Trailing)")
            render_metric_card("P/E (Trailing)", f"{pe:.1f}x" if pe else "—",
        ...
        def dark_fig(title, h=260):
            f = go.Figure()
            f.update_layout(title=title, height=h,
        ...
        with c1:
            rv = [(q.get("Revenue") or 0)/1e9 for q in quarters][::-1]
            f = dark_fig("Revenue (Billions)")
            f.add_trace(go.Bar(x=ql, y=rv, marker_color="#00d4aa"))
        ...
        with c3:
            ...
            for key, name, color in [
                ...
                f.add_trace(go.Scatter(x=ql, y=vals, name=name,
        ...
        with c4:
            ...
            fc2 = ["#26a69a" if v >= 0 else "#ef5350" for v in fv]
            f = dark_fig("Free Cash Flow (Billions)")
            f.add_trace(go.Bar(x=ql, y=fv, marker_color=fc2))
            st.plotly_chart(f, use_container_width=True)
    ...
    with cv2:
        if fund.get("summary_text"):
            ...
            <div style="background:#1a1d27;border:1px solid #2a2d3a;border-radius:8px;
              ...
              <b style="color:#00d4aa">Analysis Summary</b><br><br>{fund['summary_text']}
            </div>""", unsafe_allow_html=True)
...
with tab2:
    ...
    with cs1:
        ...
        for i in range(max(len(sup), len(res))):
            ...
            sr_rows += (f'<tr style="background:{bg}">'
    ...
    with cs2:
        ...
        for k, v in perf.items():
            color = "#ef5350" if k=="Max Drawdown" else ("#26a69a" if not str(v).startswith("-") else "#ef5350")
            st.markdown(f"""
            <div style="background:#1a1d27;border:1px solid #2a2d3a;border-radius:8px;
              ...
              <span style="color:{color};font-size:.85rem;font-family:'Courier New',monospace">{v}</span>
            </div>""", unsafe_allow_html=True)
        ...
        with cf2:
            with st.expander("🔬 Methodology"):
                ...
                <div style="font-size:.8rem;color:#8b8fa8;line-height:1.7">
                  ...
                  <b>2. Bollinger Mean-Reversion (30%)</b> — forecasts partial reversion toward 20-day rolling mean.<br><br>
                  <b>3. Forward P/E Implied (30%)</b> — forward EPS × current P/E multiple. Falls back to 55/45 split if unavailable.<br><br>
                  <b>Confidence</b> — model spread vs current price: &lt;10% = High, &lt;25% = Medium, else Low.
                </div>""", unsafe_allow_html=True)
